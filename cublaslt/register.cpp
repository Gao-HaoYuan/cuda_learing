...
// 类型转换辅助
template<typename T>
T cast_float(float v){ return static_cast<T>(v); }
...
torch::Tensor py_register_gemm_from_torch(
    ...
    // 5️⃣ 创建 GEMM Runner
    CublasLtGemmRunner<T> gemm(false, false, N, M, K);

    T alpha = cast_float<T>(1.0f);
    T beta  = cast_float<T>(0.0f); // 没有 C 输入，beta = 0
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();

    if(use_bias){
        gemm.run_with_bias(d_B, d_A, nullptr, d_D, d_bias, &alpha, &beta, stream);
    }else{
        gemm.run(d_B, d_A, nullptr, d_D, &alpha, &beta, stream);
    }
...
PYBIND11_MODULE(cublaslt_gemm, m){
    m.def("register_gemm_float", &py_register_gemm_from_torch<float>,
          py::arg("A"), py::arg("B"), py::arg("bias")=c10::nullopt);
}
